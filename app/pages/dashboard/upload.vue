  ...
  try {
    ...
    if (!failedFiles.length) {
      toast.add({
        ...
        description:
          `${successCount} tài liệu đã được tải lên`
      })
    ...
    <template #body>
      ...
      <UCard>
        ...
        <UFormField name="files">
          <UFileUpload v-model="state.files" size="md" accept=".pdf,.docx,.doc,.txt" multiple layout="list"
            label="Kéo thả các file của bạn vào đây" description="PDF, DOC, DOCX hoặc TXT" :disabled="uploading"
            highlight class="min-h-48" />
        </UFormField>
